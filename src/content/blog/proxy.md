...
| | Rotating Gateway | Static IPs |
|--|------------------|-----------|
| Pool size | Triệu IP | 10s–100s |
| Rate limit budget | Gần vô hạn | N × rps/IP |
| Latency baseline | Cao (qua internal LB) | Thấp, đường thẳng |
| Latency variance | Lớn (path khác mỗi rotation) | Nhỏ, ổn định |
| Connection pooling | Bất khả (pool key opaque, exit IP đổi liên tục) | Khả dụng |
| TLS resume với upstream | Bất khả (rotation = upstream thấy IP mới) | Khả dụng |
| Quan sát per-IP | Không | Có (monitor, cache, debug từng IP) |
| Sticky session | Có giới hạn (5–30 phút) | Vô hạn |
| Geographic precision | Thường chỉ country | City/ASN nếu provider hỗ trợ |
| Giá per GB ở scale | Cao hơn | Thấp hơn |
...
Dự án này thuộc nhóm thứ hai → static IPs + OpenResty layer tự rotate. Tóm gọn: **rotating đánh đổi latency và observability lấy pool size; static đánh đổi pool size lấy kiểm soát latency**.
